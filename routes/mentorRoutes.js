...
router.post('/updateMentorsById',async (req,res)=>{
    try{
    const updatedMentor = await mentorModel.findOneAndUpdate({id:req.body.id},req.body)
...
router.delete('/deleteMentorsById',async (req,res)=>{
    try{
    const deleteMentor = await mentorModel.findOneAndDelete({id:req.body.id})
    deleteMentor? res.send("Mentor deleted successfully") : res.status(404).send({message: "Mentor not found"})
    }
    catch(error){
        res.status(400).json(error)
    }
